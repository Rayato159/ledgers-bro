...
อัปเดต 24 กันยายน 2026: ทิศทางปัจจุบันคือเผยแพร่โอเพนซอร์ส ส่วนราคา 349 บาท การซื้อแยก Store และขั้นตอน paid download ในเอกสารนี้เป็นบันทึกแผนเก่า ไม่ใช่นโยบายปัจจุบัน ข้อกำหนด Store ต้องตรวจใหม่หากเลือกเผยแพร่ build ผ่าน Store ดู [README](../README.md) และ [ผลตรวจล่าสุด](verification-recurring-th.md)
...
เครื่องเป้าหมาย iOS ที่ผู้ใช้แจ้งเมื่อ 22 กันยายน 2026 คือ **iPhone 15 บน iOS 26.5.2** ยังไม่ได้ตรวจเวอร์ชันหรือทดสอบบนเครื่องจริง และไม่ใช่ข้อกำหนดรุ่น/iOS ต่ำสุดที่แอปรองรับ ผู้ใช้ยังไม่มี Mac; ต้องเพิ่ม iOS host กับ native adapters และเตรียม macOS + Xcode สำหรับ build/sign ก่อนติดตั้งทดสอบ

ด่านแรกบนเครื่องนี้คือ ติดตั้ง → สร้างบัญชี → บันทึกมือ → ปิด/เปิดแอปใหม่ → ตรวจยอดและรายการยังอยู่ จากนั้นตรวจอัปเกรดแอปโดยข้อมูลเดิมคงอยู่ เมื่อส่วนเชื่อมต่อพร้อมจึงทดสอบ HEIC/HEIF, OCR ไทย, เสียง และ export ผ่าน share sheet ส่วน LLM ต้องตรวจทั้งกรณีที่ Rust จัดการได้และกรณีที่เรียกโมเดลจริง วัด cold/warm latency, peak memory, การยกเลิก, การกลับจาก background และความร้อนบน iPhone โดยไม่ใช้ผล Windows/Emulator แทน ก่อนใช้เป็นสมุดบัญชีหลักต้องผ่าน backup/restore ด้วย
