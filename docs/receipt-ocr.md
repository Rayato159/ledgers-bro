...
Also run `cargo test -p ledger-infrastructure --test receipt_images -- --include-ignored` for real HEIF decoding, EXIF/container orientation, 24 MP normalization and Thai/English total recognition. Runtime-dependent integration tests are marked ignored in the portable suite and must be invoked explicitly. Fixture provenance and regeneration are in `tests/fixtures/README.md`; these images are synthetic, not captures from a physical iPhone.

To use an image taken on an iPhone, transfer it to the device running Ledgers Bro and select it with the receipt file picker. There is no iOS app. Decoder support varies by platform; if a HEIF image is rejected, convert it to JPG/PNG and review the resulting values. Synthetic-image tests do not verify every physical camera format.

The portable suite covers parser boundaries, mixed typed text, invalid values, exact reconciliation, atomic saving, retries and UI upload lifecycle. Long receipt blocks bypass the model worker's ordinary 1,000-character limit and never fall back to AI. Native OCR and file-picker verification must be recorded separately for the actual release platform.
